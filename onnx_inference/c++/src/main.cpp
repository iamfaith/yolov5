...
// ./yolo_ort -m ~/yolov5/yolov5n6-6.2.onnx -i ~/fux.png 
int main(int argc, char* argv[])
{
    ...
    if (classNames.empty() && detector.classes_.empty())
    {
        std::cerr << "Error: Empty class names file." << std::endl;
        return -1;
    }

    utils::visualizeDetection(image, result, detector.classes_.empty() ? classNames: detector.classes_);

                           ...
                           const cv::Size& inputSize = cv::Size(640, 640))
{
    ...
    if (search != nullptr)
    {
        classNames = utils::stringToVector(search.get());
    }
    search = model_metadata.LookupCustomMetadataMapAllocated("stride", allocator);
    if (search != nullptr)
    {
        stride = std::atoi(search.get()); // Convert char* to int  #include <cstdlib>
    }

    if (classNames.size() > 0)
    {
        this->classes_.clear();
        this->classes_.insert(this->classes_.end(), classNames.begin(), classNames.end());
    }
    ...
    // 从 session 获取名字
    auto inputNamesVec = session.GetInputNames();
    auto outputNamesVec = session.GetOutputNames();

    inputNameStrings.insert(inputNameStrings.end(),
                            inputNamesVec.begin(), inputNamesVec.end());
    outputNameStrings.insert(outputNameStrings.end(),
                            outputNamesVec.begin(), outputNamesVec.end());

    // std::cout << "Inputs: ";
    // for (auto& s : inputNameStrings) std::cout << s << " ";
    // std::cout << " | Outputs: ";
    // for (auto& s : outputNameStrings) std::cout << s << " ";
    // std::cout << std::endl;
                                            ...
                                            const float& iouThreshold = 0.45)
{
    ...
    for (auto& s : inputNameStrings) inputNames.push_back(s.c_str());
    for (auto& s : outputNameStrings) outputNames.push_back(s.c_str());

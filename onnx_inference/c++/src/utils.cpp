...
std::vector<std::string> utils::stringToVector(const std::string &str)
{
    ...
    {
        size_t start = item.find_first_not_of(" '"); // 找到第一个不是空格和单引号的位置
        size_t end = item.find_last_not_of(" '");    // 找到最后一个不是空格和单引号的位置
